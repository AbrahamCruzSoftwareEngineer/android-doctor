...
- ✅ Public `:core` module introduced with neutral interfaces and canonical report model.
- ✅ Premium public classes/assets/licensing/PDF export removed from public `:cli`.
- ✅ Free-only CLI flow preserved (HTML + Markdown + terminal summary).
- ✅ Public leak-prevention checks added (`verifyPublicFreeOnly`, `verifyNoPrivateCoordinates`) and wired into `check`.
- ✅ Public release gate documented in README and automated via GitHub Actions workflow.
- ⏳ Remaining: create private `android-doctor-premium` repo and publish/consume artifacts end-to-end.
...
The repository now has three Gradle modules:

- `:core` (shared model + neutral extension interfaces)
- `:plugin` (analysis and report collection)
- `:cli` (free report loading/rendering/export wiring)

`settings.gradle.kts` confirms this layout.

### Where premium code currently exists (public repo)

After the free/public migration work, premium implementations are no longer present in public source.

Current public state:

- No premium renderer classes in `:cli`.
- No licensing/entitlement implementations in public modules.
- No premium-only export implementations (e.g., PDF) in public CLI wiring.
- No premium feature routing branches in public CLI command flow.

### Coupling assessment

The architecture now has a cleaner split with a dedicated public `:core` seam and free-only public CLI wiring. This is a materially safer state for a public/free repository.

- Good: canonical report model + neutral extension interfaces in `:core`.
- Good: premium implementations/assets removed from public source.
- Remaining: private repo/artifact packaging and distribution flow still needs execution.

### Renderer separation status

Renderer separation status (public repo):

- Public renderer path is free-only.
- Premium renderer implementation is expected to live only in the future private repo.

Repository-level isolation is achieved on the public side; private-side implementation remains to be completed.
...
This phase is intentionally out-of-repo for security.
